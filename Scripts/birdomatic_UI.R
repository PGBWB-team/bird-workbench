...
footer_text <- p("Test Version: This project is powered by ",
                 ...
                 tags$a("HERE", href = "https://docs.birdomatic.com/", target = "_blank"),
                 ".  GitHub site is ",
                 tags$a("HERE", href = "https://github.com/PGBWB-team/bird-workbench", target = "_blank"),
                 ".  Offered under an ",
                 tags$a("MIT license", href = "https://mit-license.org", target = "_blank")                 
                 )
...
server <- function(input, output, session) {
  ...
  get_week_from_date <- function(date) {
	...
	week_number <- (days_since_first_sunday %/% 7) +1
		...
		# (e.g., Jan 1-3 2026 before the first Sunday of 2026 → Week 52 of 2025)
		# Note: some years (ala 2023) have 53 weeks by this reckoning
		
		if (any(week_number <= 0)) {
		  prior_year <- year - 1
		  prior_first_day <- as.Date(paste0(prior_year, "-01-01"))
		  prior_first_sunday <- prior_first_day + (7 - wday(prior_first_day) + 1) %% 7
  ...
  create_pivot_month <- function(df, yr_input = "All", loc_input = "All", species_input = NULL) {
    df <- df %>%
      ...
      mutate(Total.Count = rowSums(select(., -c(Common.Name, Species.Code)), na.rm = TRUE)) 
		...
		if (nrow(df) == 0) {
		  return(data.frame(
			Common.Name = "No matching observations. Check your filters — especially the Model-match Scale (Confidence) slider.",
			Total.Count = 0,
			Species.Code = ""
		  ))
		}  
  ...
  create_pivot_week <- function(df, yr_input = "All", loc_input = "All", week_sel, time_input = "All", species_input = NULL) {
    df <- df %>%
      filter(if (!is.null(species_input)) Common.Name %in% species_input else TRUE) %>%
#      filter(if (yr_input != "All") lubridate::year(as.Date(Date)) == yr_input else TRUE) %>%
      filter(if (yr_input != "All") SundayWeekYear == as.integer(yr_input) else TRUE) %>%# ← CHANGED THIS LINE
      filter(if (loc_input != "All") Location == loc_input else TRUE) %>%
	...
	# Use the selected year instead of current system time
    selected_year <- if (yr_input == "All") lubridate::year(Sys.time()) else yr_input
    names(week_cols) <- get_week_date_range(selected_year, week_cols, format = TRUE)
  ...
  sunday_week_year_choices <- reactive({
    years <- unique(as.character(all_data$SundayWeekYear))
    years <- years[order(years)]
    c(setNames(as.list(years), years), "All" = "All")
  }) %>% bindCache(all_data)  
  
  sunday_week_year_values <- reactive({
    # Returns just the year values (no "All") for use in plots
    years <- unique(as.character(all_data$SundayWeekYear))
    years[order(years)]
  }) %>% bindCache(all_data)  
  ...
  output$overview_view <- renderUI({
    ...
    if (input$sel_view == "by_month") {
      ...
      tagList(
        selectInput("loc_sel", label = "Location Selection", choices = loc_choices(), selected = "All"),
        selectInput("year_sel", label = "Year Selection", choices = sunday_week_year_choices(), selected = "All"),
  ...
  for (i in seq_along(location_list)) {
    local({
      ...
      output[[paste0("overviewPlot_", i)]] <- renderPlotly({
        ...
        cols <- viridis(length(sunday_week_year_choices()), direction = 1, option = "plasma")
        names(cols) <- sunday_week_year_choices()
        
		line_size <- round(seq(.3, 1.2, length.out = length(sunday_week_year_choices())), 3)
        names(line_size) <- sunday_week_year_choices()
        ...
        species_data <- species_data %>%
          mutate(
            Date = as.Date(Date),
            Year = SundayWeekYear,  # ← CHANGED: Use SundayWeekYear for weekly plots
            Month = month(Date),
            Month.Year = paste(Month, year(Date), sep = ","),  # ← Keep calendar year for monthly
            Month.Year.Loc = paste(Month, year(Date), Location, sep = ","),  # ← Keep calendar year
            Week.Year = paste(Week, SundayWeekYear, sep = ","),  # ← Use SundayWeekYear
            Week.Year.Loc = paste(Week, SundayWeekYear, Location, sep= ",")  # ← Use SundayWeekYear
          )
    ...
    local({
      ...
      output[[paste0("frequencyPlot_", i)]] <- renderPlotly({
        ...
        species_data <- species_data %>%
          mutate(
            Date = as.Date(Date),
            Year = SundayWeekYear,  # ← CHANGED: Use SundayWeekYear for weekly aggregations
            Month = month(Date),
            Month.Year = paste(Month, year(Date), sep = ","),  # ← Keep calendar year for monthly
            Month.Year.Loc = paste(Month, year(Date), Location, sep = ","),  # ← Keep calendar year
            Week.Year = paste(Week, SundayWeekYear, sep = ","),  # ← Use SundayWeekYear
        ...
        # For weekly view, Year is now SundayWeekYear
        complete_data <- species_data %>%
          complete(Year = unique(species_data$Year), Week = 1:53, fill = list(Count = 0))
        # Note: This is correct as-is since we already set Year = SundayWeekYear above
        ...
        recent_years <- tail(sort(unlist(sunday_week_year_choices())), 4)
  ...
  observe({
    ...
    output$filtered_data <- DT::renderDT({
      ...
      species_data <- species_data %>%
        mutate(
          Date = as.Date(Date),
          Year = if(input$time_interval == "weekly") SundayWeekYear else year(Date),  # ← CHANGED: Context-aware
          Month = month(Date),
          Month.Year = paste(Month, year(Date), sep = ","),  # ← Keep calendar year for monthly
          Month.Year.Loc = paste(Month, year(Date), Location, sep = ","),  # ← Keep calendar year
          Week.Year = paste(Week, SundayWeekYear, sep = ","),  # ← Use SundayWeekYear for weekly keys
          Week.Year.Loc = paste(Week, SundayWeekYear, Location, sep= ",")  # ← Use SundayWeekYear
        )
